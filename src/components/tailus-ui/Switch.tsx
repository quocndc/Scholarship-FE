...
const SwitchRoot = React.forwardRef<React.ElementRef<typeof Switch.Root>, React.ComponentPropsWithoutRef<typeof Switch.Root> & SwitchVariantsProps>(
...
const SwitchThumb = React.forwardRef<React.ElementRef<typeof Switch.Thumb>, React.ComponentPropsWithoutRef<typeof Switch.Thumb> & SwitchProps>(
  ({ className, ...props }, forwardedRef) => {
    const { intent } = useContext(SwitchContext);
    const { thumb } = switchTheme({ intent });
    return <Switch.Thumb className={thumb({ intent, className })} {...props} ref={forwardedRef} />;
  }
);
