...
const ToggleRoot = React.forwardRef<React.ElementRef<typeof Root>, React.ComponentPropsWithoutRef<typeof Root> & ToggleRootProps>(
  ({ className, variant = 'soft', size = 'md', intent = 'primary', withLabel = false, ...props }, forwardedRef) => {
    return <Root className={root({ variant, size, withLabel, intent, className })} ref={forwardedRef} {...props} />;
  }
);
